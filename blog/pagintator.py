...
class Paingator:
        ...
        for i in range(self.page_start_num, self.page_end_num + 1):
            ...
            if i == self.current_page_num:
                page_link_list.append(
                    '<a class="page active" href="%s?%s">%s</a>' % (self.base_url, self.params.urlencode(), i))
            else:
                page_link_list.append(
                    '<a class="page"href="%s?%s">%s</a>' % (self.base_url, self.params.urlencode(), i))
    ...
    def bootstrap_page_html(self):
        ...
        for i in range(self.page_start_num, self.page_end_num + 1):
            self.params['page'] = i
            if i == self.current_page_num:
                page_link_list.append('<li class="active"><a class="page" href="%s?%s">%s</a></li>' % (
                self.base_url, self.params.urlencode(), i))
            else:
                page_link_list.append(
                    '<li><a class="page"href="%s?%s">%s</a></li>' % (self.base_url, self.params.urlencode(), i))
        page_link_list = ''.join(page_link_list)
        return page_link_list
